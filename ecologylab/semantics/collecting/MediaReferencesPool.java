...
/**
 * A PrioritizedPool of references to Media, like Images. A thread wakes up periodically and parses
 * the top ranked one.
 * 
 * @author andruid
 */
		...
		Runnable, RunnablePool
{
	...
	/**
	 * How long the agent plans to sleep for in the current / next iteration. Also used as a
	 * comparison to see if sleep time has been decreased.
	 */
	int																										sleep;
	...
	/**
	 * This lock is for start, stop, pause, and unpause. prune() takes the lock on <code>this</code>,
	 * so we need a separate one here.
	 */
	final Object																					startAndStopLock								= new Object();

	/**
	 * Controls whether or not we periodically automatically download the ImgElements that have been
	 * discovered. In other words, controls whether or not we collect images that we know about. Set
	 * as a preference at runtime, and via a menu entry.
	 */
	PrefBoolean																						downloadImagesAutomatically			= Pref
																																														.usePrefBoolean(
																																																"download_images_automatically",
																																																true);
