...
/**
	 * Super class for all document types.
	 * This class obtains the connection to a document.
	 * A parse method may be present to process the document.
	 * <p/>
	 * Their role is to parse documents. They start with a PURL, which the static connect() method translates into 
	 * an PURLConnection, and an appropriate instance of a subclass.
	 * The {@link #parse() parse()} method is then called to translate the document into
	 * the semantic model, using the PURLConnection.
	 * <p/>
	 * The translation from a PURL to an appropriate subclass instance is performed using a combination of the
	 * PURLs extension, if it has a useful one, and then the mime-type returned by the URLConnection response header.
	 * These keys are then used to perform a lookup in one of the registries maintained in this class.
	 * 
	 * @author andruid
	 * @author eunyee
	 */
	...
	extends Debug
	{
		...
		//private static final ClassRegistry<? extends DocumentType>	rbs		= new ClassRegistry<? extends DocumentType>();
		...
		/**
		 * Open a connection to the URL. Read the header, but not the content.
		 * Look at if the path exists, if there is a redirect, and the mime type.
		 * If there is a redirect, process it.
		 * <p/>
		 * Create an InputStream.
		 * Using reflection (Class.newInstance()), 
		 * create the appropriate DocumentType, based on that mimeType, using
		 * the allTypes HashMap. Return it.
		 */
		public static DocumentType connect(final ParsedURL purl, 
				final AbstractContainer container, final InfoProcessor infoCollector)
		{
			DocumentTypeHelper helper	= new DocumentTypeHelper()
			{
				...
				public boolean processRedirect(URL connectionURL) throws Exception
				{
					...
					AbstractContainer redirectedAbstractContainer	= infoCollector.lookupAbstractContainer(connectionPURL);   
					if (redirectedAbstractContainer != null)
					{
						// the redirected url has been visited already.
						// add this url into the redirected AbstractContainers's additionalURLs.
						if (container != null)
					...
					{  
						...
						if (infoCollector.accept(connectionPURL))
						{
							...
							if (container != null)
							{
								...
								//The redirected URL has a timeout...which creates a problem while 
								//opening the saved xml.
								//							if(connectionPURL.toString().startsWith("http://delivery.acm.org"))
								//							{
								//							return true;
								//							}
							...
							// this is the only redirect case in which we continue processing
							return true;
						}
						else
			...
			PURLConnection purlConnection = purl.connect(helper, (metaMetadata == null) ?  null : metaMetadata.getUserAgentString());
			DocumentType result						= helper.getResult();

			//		if(purlConnection.toString().startsWith("http://portal.acm.org/citation.cfm?id=336478"))
			////		if(purl.toString().startsWith("http://portal.acm.org/citation.cfm"))
			//		{
			//		println("debug");
			//		}
			// In some cases, the documentType is preset (e.g., GoogleSearch, FlickrSearch, ...).
			// So, in this case, set the result now, instead of divining it from mimeType.
			...
			if ((purlConnection != null) && (result == null))
			{
				 ...
				 * one of the prefix has to be acm's. The getLookupPurl() method takes in 
				 * the purl and returns the prefix purl. This prefix purl has to be used 
				 * in lookupSpecialExtractor() to get the desired document type.
				 */
				//			if(acmPrefixCollection.match(purl.toString(), '?'))
				//FIXME -- Abhinav -- get rid of this SOON. Your type will supercede it.
//				ParsedURL lookupPurl = getLookupPurl(purl);
//				result						= lookupSpecialExtractor(lookupPurl, infoCollector);
				...
				if (result == null)
				{
					...
					if (result == null)
					{
						//ACMPortal type document is decided based on mimeType which is text/html
...
//			else 
		...
		/**
		 * True if our analysis indicates the present AbstractContainer is an article, and not
		 * a collection of links.
		 * This affects calls to getWeight() in the model!
		 * 
		...
		/*
		 * This method is an odd out for File-based instances of DocumentType,
		 * that arise during parse() of FileDirectoryType.
		 * 
		...
		/**
		 * Set the inputStream of this documeType object. 
		 * Called from HTMLFragment.
		 * @param inputStream
		 */
		...
		/**
		 * Get the ParsedURL value of the current document. 
		 * If you want to get a URL value, you can use url() method in ParsedURL. 
		 * @return
		 */
		 ...
		 */
		protected static void registerMime(String mimeType, Class<? extends DocumentType> documentTypeClass)
		{
		...
		/**
		 * Create a mapping from filename suffix to the class object for a DocumentType.
		 * Actually create two mappings, one lower case, and one upper case.
		 */
		protected static void registerSuffix(String suffix, Class<? extends DocumentType> documentTypeClass)
		{
		...
		{
			return ((suffix == null) || (suffix.length() == 0)) ? null : getInstanceFromRegistry(registryBySuffix, suffix, infoCollector);
		}

		/**
		 * Find the DocumentType class that corresponds to the documentTypeSimpleName, and
		 * construct a fresh instance of that type.
		 * 
		 * @param documentTypeSimpleName - Name of the DocumentType subclass, without the package.
		 * @param infoCollector TODO
		 * 
		 * @return	an instance of the DocumentType subclass that corresponds to documentTypeSimpleName.
		 */ 
		public static DocumentType getInstanceBySimpleName(String documentTypeSimpleName, InfoProcessor infoCollector)
		{
		...
		/**
		 * Given one of our registries, and a key, do a lookup in the registry
		 * to obtain the Class object for the DocumentType subclass corresponding
		 * to the key -- in that registry.
		 * <p/>
		 * Then, construct an instance of that object
		 * @param infoCollector TODO
		 * 
		 * @return		an instance of the DocumentType subclass that corresponds to
		 *				the key, in the specified registry.
		 */
		public static<DT extends DocumentType> DT getInstanceFromRegistry(Scope<Class<? extends DT>> thatRegistry,
				String key, InfoProcessor infoCollector)
		{
			DT result		= null;
			...
			result = ReflectionTools.getInstance(documentTypeClass, infoCollector.getInfoProcessorClassArg(), constructorArgs);
			return result;
		}
		 ...
		 * 
		 * @param elementState ElementState object with a TEXT Node for description.
		...
		public static boolean isNonJunkDescription(String description)
		{
			...
			!description.startsWith("Find breaking") && // nytimes
			!description.startsWith("Read full") &&	 // cnn
		...
		/**
		 * Obtain an instance of an information extractor, based on a URL that takes arguments,
		 * if one is available.
		 * (The ParsedURL arguments, found after ? in the URL, are ignored for the matching.)
		 * 
		 * @param key
		 * @param thatMap
		 * @return
		 */
		public static DocumentType getInstanceFromMap(ParsedURL purl, HashMap<String, Class<? extends DocumentType>> thatMap)
		{
			DocumentType result		= null;
			Class<? extends DocumentType> documentTypeClass	= thatMap.get(purl.noAnchorNoQueryPageString());

			result = ReflectionTools.getInstance(documentTypeClass);
			return result;
		}

		public static DocumentType getInstanceFromMap(ParsedURL purl, HashMap<String, Class<? extends DocumentType>> thatMap, InfoProcessor infoCollector)
		{
			DocumentType result		= null;
			Class<? extends DocumentType> documentTypeClass	= thatMap.get(purl.noAnchorNoQueryPageString());
